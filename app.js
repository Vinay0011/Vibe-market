...
async function sbGet(key) {
  const r = await fetch(
    `${SUPABASE_URL}/rest/v1/vm_data?key=eq.${key}&select=value`,
    { headers: SB_HEADERS }
  );
  const d = await r.json();
  return (d[0] && d[0].value) ? d[0].value : [];
}
...
async function loadDB() {
  try {
    ...
    toast('⚠️','Connection issue','Could not load data. Check your Supabase config in app.js.','err');
    DB_LOADED = true;
  }
}
...
async function saveDB() {
  try {
    ...
    toast('⚠️','Save failed','Could not save. Check Supabase config.','err');
  }
}
...
async function tgSend(text) {
  ...
  } catch(e) { console.warn('TG fail', e); }
}
...
async function cmdApprove(id) {
  ...
  tgSend(`✅ <b>APPROVED!</b>\n📦 <b>${item.name}</b> is now LIVE at the top of VibeMarket!\n🔗 ${item.url||'—'}\n👤 ${item.contact}`);
}
...
// ─── RATE LIMIT: 1 listing per person per day ──────────────────────────────────
...
class P {
  constructor() { this.re(); }
  re() { this.x=Math.random()*W; this.y=Math.random()*H; this.vx=(Math.random()-.5)*.25; this.vy=(Math.random()-.5)*.25; this.r=Math.random()*1.3+.2; this.a=Math.random()*.42+.08; this.c=Math.random()>.65?'#00ffe0':Math.random()>.5?'#bf00ff':'#fff'; }
  up() { this.x+=this.vx; this.y+=this.vy; if(this.x<0||this.x>W||this.y<0||this.y>H) this.re(); }
  dr() { ctx.beginPath(); ctx.arc(this.x,this.y,this.r,0,Math.PI*2); ctx.fillStyle=this.c; ctx.globalAlpha=this.a; ctx.fill(); ctx.globalAlpha=1; }
}
...
(function loop() {
  ...
  for (let i=0; i<pts.length; i++) for (let j=i+1; j<pts.length; j++) {
    ...
    if (d<90) { ctx.beginPath(); ctx.moveTo(pts[i].x,pts[i].y); ctx.lineTo(pts[j].x,pts[j].y); ctx.strokeStyle='#00ffe0'; ctx.globalAlpha=(1-d/90)*.04; ctx.lineWidth=.4; ctx.stroke(); ctx.globalAlpha=1; }
  }
...
// ─── HELPERS ──────────────────────────────────────────────────────────────────
function tbadge(t) {
  const map = { website:['tb-w','Website'], webapp:['tb-a','Web App'], saas:['tb-w','SaaS'], tool:['tb-a','Tool'], other:['tb-b','Other'] };
...
// ─── RENDER LISTINGS ──────────────────────────────────────────────────────────
function renderListings(filter = 'all', search = '') {
  const grid = document.getElementById('grid'); grid.innerHTML = '';
  const items = DB.listings.filter(d => {
    const fm = filter === 'all' || d.ptype === filter;
    const sm = !search || d.name.toLowerCase().includes(search) || (d.desc||'').toLowerCase().includes(search) || (d.tagline||'').toLowerCase().includes(search);
    return fm && sm;
  });
  if (!items.length) {
    grid.innerHTML = `<div class="empty-st"><div class="es-ico">🌐</div><div class="es-t">No projects yet</div><div class="es-s">Be the first — <a href="#list">submit yours for free →</a></div></div>`;
    return;
  }
...
document.querySelectorAll('.fb').forEach(btn => {
  btn.addEventListener('click', () => {
    ...
    renderListings(btn.dataset.f, document.getElementById('srch').value.toLowerCase());
  });
});
document.getElementById('srch').addEventListener('input', e => {
  renderListings(document.querySelector('.fb.on').dataset.f, e.target.value.toLowerCase());
});
...
function openDetail(id, source) {
  ...
  const tmap = { website:['tb-w','Website'], webapp:['tb-a','Web App'], saas:['tb-w','SaaS'], tool:['tb-a','Tool'], other:['tb-b','Other'] };
  const [cls, lbl] = tmap[item.ptype || 'other'] || ['tb-b','Other'];
  tb.className = `tb ${cls}`; tb.textContent = lbl;
  ...
  // Info grid
  const infoItems = [];
  if (item.url)     infoItems.push(['Live URL', `<a href="${item.url}" target="_blank" rel="noopener">${item.url}</a>`]);
...
function cp(text, btn) {
  navigator.clipboard.writeText(text).then(() => {
    const o = btn.textContent; btn.textContent = 'COPIED!'; btn.style.color = 'var(--neon)'; btn.style.borderColor = 'var(--neon)';
...
function toast(icon, title, sub, type = '') {
  ...
  clearTimeout(_tt); _tt = setTimeout(() => el.classList.remove('show'), 5500);
}
...
function checkLimitNotice() {
  ...
  if (!canSubmitToday()) {
    ...
    document.querySelector('.list-form .btn-p').disabled = true;
    document.querySelector('.list-form .btn-p').style.opacity = '.4';
    document.querySelector('.list-form .btn-p').style.cursor  = 'not-allowed';
  } else {
...
async function submitListing() {
  ...
  const rawFeats = document.getElementById('lfeats').value;
  const feats    = rawFeats.split('\n').map(s => s.trim()).filter(Boolean);
  ...
  ['ln','lurl','ltagline','ltags','lico','ldesc','lfeats','lrev','lusers','lct'].forEach(id => document.getElementById(id).value = '');
  btn.disabled = false; btn.textContent = 'PUBLISH FOR FREE →';

  renderListings(); updateStats(); checkLimitNotice();
...
async function submitPromo() {
  ...
  const item = {
    ...
    icon: '⚡', tags: ['Featured'],
    contact: ct, tx, chain,
    tagline: desc, features: [], status:'live', built:'',
    createdAt: Date.now(),
  };
...
// ─── SHAKE ────────────────────────────────────────────────────────────────────
function shake(el) { el.style.animation = 'shk .38s ease'; setTimeout(() => el.style.animation = '', 400); }
...
async function init() {
  ...
  document.getElementById('grid').innerHTML = `
    <div class="loading-st">
      ...
      <div class="loading-txt">Loading projects…</div>
    </div>`;
